...
import { MAX_IMPORT_ROWS, MAX_SCANNED_HEADER_ROWS } from './import.limits';
import { fieldForHeader, headerScore, isIgnoredHeader, normalizeHeader, REQUIRED_FIELDS, REQUIRED_LABELS, type ImportField } from './import.columns';

/**
 * Turning an uploaded spreadsheet into plain strings.
 *
...
export interface ParsedSheet {
  ...
  mapping: { header: string; field: ImportField | null; ignored: boolean }[];
 ...
 *
 * The product export writes `'=1+1` so Excel cannot execute the cell. Importing
 * that same file back must restore `=1+1` rather than storing the apostrophe in
 * a product name. Only the characters the exporter neutralises are unwrapped.
 */
...
async function readWorksheet(buffer: Buffer, format: ImportFormat): Promise<ExcelJS.Worksheet> {
  ...
  // The product export writes one sheet per section; the first one carries the products.
 ...
 *
 * A file straight from the product export starts with a title block (workspace,
 * dataset, generated-at, filters) and a blank line before the headers, so the
 * first rows are scanned for the row that names the most import columns. This
 * is what makes "export → edit → import" work with no manual tidying.
 */
function findHeaderRow(sheet: ExcelJS.Worksheet, width: number): { rowNumber: number; cells: string[] } {
  ...
  for (let rowNumber = 1; rowNumber <= last; rowNumber += 1) {
    ...
    if (score >= REQUIRED_FIELDS.length && cells.filter((cell) => fieldForHeader(cell)).length >= REQUIRED_FIELDS.length) break;
  }
  return { rowNumber: best.rowNumber, cells: best.cells };
}

export async function parseProductSheet(buffer: Buffer, format: ImportFormat): Promise<ParsedSheet> {
  ...
  if (header.rowNumber === 0) {
    throw ApiError.badRequest(`The file has no recognisable header row. It needs the columns: ${REQUIRED_LABELS.join(', ')}.`);
  }

  const headers = header.cells;
  const mapping = headers
    .map((text) => ({ header: text, field: fieldForHeader(text), ignored: isIgnoredHeader(text) }))
    .filter((entry) => entry.header !== '');

  // A field may be named only once; two "Price" columns would be a silent guess.
  const seen = new Map<ImportField, string>();
  ...
  if (missing.length > 0) {
    const labels = missing.map((field) => REQUIRED_LABELS[REQUIRED_FIELDS.indexOf(field)]);
  ...
  for (let rowNumber = header.rowNumber + 1; rowNumber <= sheet.rowCount; rowNumber += 1) {
    ...
    if (rows.length >= MAX_IMPORT_ROWS) {
      throw ApiError.badRequest(
        `This file has more than ${MAX_IMPORT_ROWS.toLocaleString('en-US')} product rows. Split it into smaller files and import them one after another.`,
  ...
  if (rows.length === 0) throw ApiError.badRequest('The file has headers but no product rows.');
