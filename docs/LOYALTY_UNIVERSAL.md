...
Universal POS task 09, part 1: the card program that Clothing has always had now runs in a
Super Shop till too. Pharmacy and Restaurant are part 2 — see *Not yet* at the end.
...
- **Restaurant** — earns at order level, and its refunds are money-only, so the hook belongs on
  payment rather than on the order.
